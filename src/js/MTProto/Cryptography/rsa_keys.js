...
function prepare() {
    for (let i = 0; i < keys.length; i++) {
        ...
        rsaPublicKeySerializer.bytes(Uint8.fromHex(keyParsed.modulus))
        rsaPublicKeySerializer.bytes(Uint8.fromHex(keyParsed.exponent))

        let bytes = rsaPublicKeySerializer.toUint8Array();

        let fingerprintBytes = sha1(bytes).slice(-8)
        fingerprintBytes.reverse()

        parsedKeys[Uint8.toHex(fingerprintBytes)] = {
