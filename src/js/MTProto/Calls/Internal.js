...
export const CallsInternal = {
    startCall(dhConfig) {
        const gA = Uint8.fromBigInteger(BigInteger(dhConfig.g).modPow(Uint8.toBigInteger(dhConfig.random), Uint8.toBigInteger(dhConfig.p)))
    ...
    getAuthKey(gB, random, p) {
        const authKey = Uint8.modPow(gB, random, p)
    ...
    generateEmojiFingerprint(authKey, gA) {
        const hash = new Uint8Array(sha256(Uint8.concat(new Uint8Array(authKey), new Uint8Array(gA))))
        const result = []
        const emojiCount = this.emojiOffsets.length - 1
        const kPartSize = 8;
        for (let partOffset = 0; partOffset !== hash.byteLength; partOffset += kPartSize) {
            let value = this.computeEmojiIndex(hash.subarray(partOffset, partOffset + kPartSize));
            let index = value.modPow(1, emojiCount).toJSNumber();
            let offset = this.emojiOffsets[index];
            let size = this.emojiOffsets[index + 1] - offset;
    ...
    computeEmojiIndex(bytes) {
        return ((BigInteger(bytes[0]).and(0x7F)).shiftLeft(56))
            .or((BigInteger(bytes[1]).shiftLeft(48)))
            .or((BigInteger(bytes[2]).shiftLeft(40)))
            .or((BigInteger(bytes[3]).shiftLeft(32)))
            .or((BigInteger(bytes[4]).shiftLeft(24)))
            .or((BigInteger(bytes[5]).shiftLeft(16)))
            .or((BigInteger(bytes[6]).shiftLeft(8)))
            .or((BigInteger(bytes[7])))
    },

    emojiData: new Uint16Array([
