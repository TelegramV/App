...
export function mt_file_write_bytes(offset, length, input, buffer)
{
...
export function mt_file_write_uint32(offset, value, buffer)
{
    buffer.setUint32(offset, value, true);
}

function mt_file_read_uint32(offset, buffer)
{
    return buffer.getUint32(offset, true);
}

function mt_file_write_uint64(offset, left, right, buffer)
{
    buffer.setUint32(offset, left, true);
    buffer.setUint32(offset + 4, right, true);
}

function mt_file_get_random_num_secure(max) {
...
mt_file_get_random_num_secure is very recommended for crypto
*/
function mt_file_get_random_num_unsecure(max) {
    max = Math.floor(max);
...
function mt_file_init_obfuscation(out_buffer_view, url)
{
    ...
    for(;;)
    {
        var f = mt_file_get_random_num_secure(0xFFFFFFFF);
        var s = mt_file_get_random_num_secure(0xFFFFFFFF);

        if((f & 0xFF) != 0xef && f != 0xdddddddd && f != 0xeeeeeeee
        && f != 0x504f5354 && f != 0x474554 && f != 0x48454144 && s != 0x00000000)
        {
            mt_file_write_uint32(obfuscation_buffer_offset, f, obfuscation_buffer_view);
            obfuscation_buffer_offset += 4;
            mt_file_write_uint32(obfuscation_buffer_offset, s, obfuscation_buffer_view);
            obfuscation_buffer_offset += 4;
            break;
        }
    }
    for(var i = 0; i < 12; ++i)
    {
        mt_file_write_uint32(obfuscation_buffer_offset, mt_file_get_random_num_secure(0xFFFFFFFF), obfuscation_buffer_view);
        obfuscation_buffer_offset += 4;
    }

    mt_file_write_uint32(obfuscation_buffer_offset, 0xeeeeeeee, obfuscation_buffer_view);
    obfuscation_buffer_offset += 4;
    ...
    obfuscation_buffer_view.setUint16(obfuscation_buffer_offset, mt_file_get_random_num_secure(0xFFFF));
    ...
    // return AppCryptoManager.set_aes_decryptor(url, deobf_key_256, deobf_vector_128)
}

function mt_file_inob_send_init(socket, url)
{
    var obf_buffer = new ArrayBuffer(64);
    var obf_buffer_view = new DataView(obf_buffer);

    // mt_file_init_obfuscation(obf_buffer_view, url).then (() => {
    //     socket.send(obf_buffer);
    //     obfuscation_init[url] = true;
    // })
    mt_file_init_obfuscation(obf_buffer_view, url)
    socket.send(obf_buffer);
    obfuscation_init[url] = true;

}

export function mt_file_inob_clear(url) {
    delete obfuscation_init[url]
    delete aes_encryptors[url]
    delete aes_decryptors[url]
}

export function mt_file_inob_send(socket, buffer, buffer_len, url)
{
    if(!obfuscation_init[url])
    {
        mt_file_inob_send_init(socket, url);
    }
    ...
    mt_file_write_uint32(0, buffer_len, out_buffer_view);
    mt_file_write_bytes(4, buffer_len, new Uint8Array(buffer), out_buffer_view);

    var encrypted_buffer = aes_encryptors[url].encrypt(new Uint8Array(out_buffer));
    socket.send(encrypted_buffer);
}

export async function mt_file_inob_recv(ev, url)
{
    if(!obfuscation_init[url])
    {
        return null;
    }
    console.time("file decrypt")

    // var decrypted_buffer = await AppCryptoManager.aes_decryptor(url, ev.data)
    var decrypted_buffer = aes_decryptors[url].decrypt(new Uint8Array(ev.data)).buffer

    console.timeEnd("file decrypt")
