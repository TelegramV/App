var oldWebkit = false;
var oldIE = false;
if (crypto && crypto.webkitSubtle && crypto.subtle === undefined) {
    oldWebkit = true;
}

var _crypto = crypto;
var _cryptoSubtle = _crypto.subtle || _crypto.webkitSubtle;
var _indexedDB = indexedDB || msIndexedDB;

var config = {
    aesCipher: "TELEGRAM_CRYPTO-CBC",
    aesLength: 128,
    aesIvLength: 16,
    hmacOptions: {
        name: "HMAC",
...
var simpleCrypto = {
    ...
    asym: {
                ...
                internal.asym.exportKey(keys.publicKey, ["encrypt"], onError.bind(null, "Could not export public encrypt key"), function (publicJwk) {
                    internal.asym.exportKey(keys.privateKey, ["decrypt"], onError.bind(null, "Could not export private encrypt key"), function (privateJwk) {
                        internal.asym.importEncryptKey(privateJwk, ["decrypt"], onError.bind(null, "Could not import private encrypt key"), function (privateKey) {
                            ...
                            onSuccess({
                                privateKey: privateKey, publicKey: keys.publicKey,
                                privateJwk: privateJwk, publicJwk: publicJwk
                            });
        ...
        /**
         * Decrypt. For large payloads, keys are RSA decrypted and content is TELEGRAM_CRYPTO decrypted. Otherwise, content is RSA decrypted.
         *
         * @method asym.decrypt
        ...
        decrypt: function (decryptKey, dict, onError, onSuccess) {
            if (dict.aesEncrypted) {
                internal.asym.decrypt(decryptKey, dict.rsaEncrypted, onError.bind(null, "Could not decrypt keys"), function (combinedKeys) {
                    internal.asym.aesDecrypt(combinedKeys, {
                        aesEncrypted: dict.aesEncrypted,
                        hmac: dict.hmac
                    }, onError, onSuccess);
                });
            } else {
        ...
        /**
         * Verify and Decrypt. For large payloads, TELEGRAM_CRYPTO keys are RSA decrypted and content is TELEGRAM_CRYPTO decrypted. Otherwise, content is RSA decrypted.
         * The order of operations is: verify encrypted content/keys, decrypt, verify decrypted content/keys, TELEGRAM_CRYPTO decrypt (if required).
         *
            ...
            internal.asym.verifySignature(verifyKey, dict.signatureOfEncrypted, dict.rsaEncrypted, onError.bind(null, "Could not verify encrypted keys"), function () {
                internal.asym.decrypt(decryptKey, dict.rsaEncrypted, onError.bind(null, "Could not decrypt keys"), function (decrypted) {
                    internal.asym.verifySignature(verifyKey, dict.signatureOfData, decrypted, onError.bind(null, "Could not verify keys"), function () {
                        if (dict.aesEncrypted) {
                            internal.asym.aesDecrypt(decrypted, {
                                aesEncrypted: dict.aesEncrypted,
                                hmac: dict.hmac
                            }, onError, onSuccess);
                        } else {
        ...
        /**
         * Encrypt. For large payloads, the content is TELEGRAM_CRYPTO encrypted and keys are RSA encrypted. Otherwise, the content is RSA encrypted.
        ...
        /**
         * Encrypt and Sign. For large payloads, the content is TELEGRAM_CRYPTO encrypted and keys are RSA encrypted. Otherwise, the content is RSA encrypted.
         * "Large" trashhold is defined in config.
         * The order of operations is: TELEGRAM_CRYPTO (encrypt if required), encrypt content/keys, sign content/keys, sign encrypted content/keys.
         *
    ...
    sym: {
        ...
        /** Generate random TELEGRAM_CRYPTO and HMAC keys
         *
        ...
        generateKeys: function (onError, onSuccess) {
            ...
            internal.sym.generateKeyAES(onError.bind(null, "Could not generate TELEGRAM_CRYPTO key"), function (aesKeyObj) {
                internal.sym.generateKeyHMAC(onError.bind(null, "Could not generate HMAC key"), function (hmacKeyObj) {
                    internal.sym.exportKey(aesKeyObj, onError.bind(null, "Could not export TELEGRAM_CRYPTO key"), function (aesKey) {
                        internal.sym.exportKey(hmacKeyObj, onError.bind(null, "Could not export HMAC key"), function (hmacKey) {
                            internal.sym.importKeyAES(aesKey, onError.bind(null, "Could not import TELEGRAM_CRYPTO key"), function (aesKeyObjNotExtract) {
                                internal.sym.importKeyHMAC(hmacKey, onError.bind(null, "Could not import HMAC key"), function (hmacKeyObjNotExtract) {
                                    onSuccess({
                                        aesKeyObj: aesKeyObjNotExtract,
                                        hmacKeyObj: hmacKeyObjNotExtract,
                                        aesKey: aesKey,
                                        hmacKey: hmacKey
                                    });
                                });
        ...
        /** Convert raw TELEGRAM_CRYPTO and HMAC keys into WebCrypto CryptoKey objects
         *
        ...
        importKeys: function (keys, onError, onSuccess) {
            ...
            internal.sym.importKeyAES(keys.aesKey, onError.bind(null, "Could not import TELEGRAM_CRYPTO key"), function (aesKeyObj) {
        ...
        /** Generate random TELEGRAM_CRYPTO and HMAC keys and encrypt data. This utility function combines generateKeys and encrypt
         *
         ...
         *
         * @method sym.encrypt
         * @param {Object} keys - TELEGRAM_CRYPTO and HMAC keys. Also optional IV (otherwise random is generated)
         * @param {Uint8Array | ArrayBuffer} data - Data to encrypt
         * @param {function} onError - called with error details if key generation or encryption fail
        ...
        encrypt: function (keys, data, onError, onSuccess) {
            ...
            simpleCrypto.sym.importKeys(keys, onError, function () {
                ...
                internal.sym.encrypt(keys.aesKeyObj, iv, data, onError.bind(null, "Could not TELEGRAM_CRYPTO Encrypt"), function (aesEncrypted) {
         ...
         * @param {string} password - Password used to generate TELEGRAM_CRYPTO and HMAC keys
         * @param {Uint8Array | ArrayBuffer} data - Data to encrypt
         * @param {function} onError - called with error details if key generation or encryption fail
         * @param {function} onSuccess - called with {aesEncrypted: ArrayBuffer, hmac: {ArrayBuffer}
         */
        ...
        decrypt: function (keys, encrypted, onError, onSuccess) {
            simpleCrypto.sym.importKeys(keys, onError, function () {
                internal.sym.verifyHMAC(keys.hmacKeyObj, encrypted.hmac, encrypted.aesEncrypted, onError.bind(null, "Could not verify HMAC"), function () {
                    internal.sym.decryptAES(keys.aesKeyObj, encrypted.aesEncrypted, onError.bind(null, "Could not TELEGRAM_CRYPTO decrypt"), function (decrypted) {
                        onSuccess(decrypted);
                    });
                });
            });
        ...
        decryptWithPassword: function (password, encrypted, onError, onSuccess) {
            simpleCrypto.pbkdf2.derive(password, 288, {salt: encrypted.pbkdf2_salt, iterations: encrypted.pbkdf2_iter},
                onError.bind(null, "PBKDF2 error"), function (pbkdf2) {
                    ...
                    var aesKey = keys.subarray(0, aesKeyLenght);
                    var hmacKey = keys.subarray(aesKeyLenght);
                    simpleCrypto.sym.decrypt({
                        aesKey: aesKey,
                        hmacKey: hmacKey
                    }, encrypted, onError, function (decrypted) {
                        onSuccess(decrypted);
                    });
                }
            );
