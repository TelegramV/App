...
class ServiceMessageComponent extends GeneralMessageComponent {
    ...
    componentDidMount() {
        ...
        let photo = this.props.message?.action?.photo;
        if(photo) {
            FileManager.downloadPhoto(photo);
        }
    }

    appEvents(E) {
        E.bus(AppEvents.Files)
        .filter(event => event.file === this.props.message?.action?.photo)
        .on("download.done", this.photoReady);
    }
    ...
    photoReady = (event) => {
        this.$el.querySelector(".photo").src=event.url;
    }
    ...
    wrapPeer = (peer) => {
        ...
        return (<span class="clickable" onClick={_=>{AppSelectedInfoPeer.select(peer)}}>{peer.name}</span>);
    }

    wrapPeerWithSelf = (peer) => {
        if(!peer) return "UNKNOWN USER";
        let text = peer.isSelf ? "You" : peer.name;
        return (<span class="clickable" onClick={_=>{AppSelectedInfoPeer.select(peer)}}>{text}</span>);
    }
}
