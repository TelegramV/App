...
export const computeEmojiFingerprint = (h) => {
    ...
    for (let partOffset = 0; partOffset !== hash.byteLength; partOffset += kPartSize) {
        let value = computeEmojiIndex(hash.subarray(partOffset, partOffset + kPartSize));
        let index = value.modPow(1, emojiCount).toNumber();
        let offset = emojiOffsets[index];
        let size = emojiOffsets[index + 1] - offset;
...
const emojiData = new Uint16Array([
