...
const handlersLinks = {
    ...
    messageEntityUrl: (l, a, q) => <a target="_blank" href={q.match(/^[a-zA-Z]+:\/\//) ? q : 'https://' + q}>{a}</a>,
    messageEntityEmail: (l, a, q) => <a href={`mailto:${q}`}>{a}</a>,
    messageEntityTextUrl: (l, a) => <a target="_blank" href={l.url.match(/^[a-zA-Z]+:\/\//) ? l.url : 'https://' + l.url}>{a}</a>,
